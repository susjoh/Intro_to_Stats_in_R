...
\begin{figure}[h]
\centering 
\includegraphics[width=1\textwidth]{figs/MarkdownTemplatev2.JPG}
...
\texttt{date:} ``\texttt{05 November 2019}'' \\
...
\begin{enumerate}
\item Add a code chunk for loading and manipulating the data above, and running the $\chi^{2}$ test.
...
2. Use the Markdown Quick Reference in RStudio to add the image of \textit{Iris virginica} to the Markdown document (``\texttt{data/Irisvirginica.jpg}'')
...
Hint: extracting values from the model can use indexing as visited in the preparation tutorial for the previous session. The best approach is to create an object for the summary of lm.heights e.g. \code{summary.lm.heights <- summary(lm.heights)\$coefficients} and calling values based on the indices (e.g. the t-value will be \code{summary.lm.heights[2,3]}.
\end{minipage}}
